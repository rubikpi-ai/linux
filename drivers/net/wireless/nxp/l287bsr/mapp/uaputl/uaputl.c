 ...
 *  @param custom_bcn Update beacon or not
		       ...
		       t_u8 custom_bcn)
{
	...
	if (custom_bcn)
		fp = fopen("config/80211d_country_ie.conf", "r");
	else
	...
	if (!found) {
		if (custom_bcn)
			printf("No match found for Country = %s in the 80211d_country_ie.conf \n",
			       country);
		else
	...
	while (!feof(fp)) {
		...
		if (dflag) {
			...
			default:
				if (custom_bcn)
					printf("ERR: Incorrect 80211d_country_ie.conf file\n");
				else
...
static void print_hscfg_usage(void)
{
	...
	printf("\n              or 0xff for special setting (host acknowledge required)");
...
static int apcmd_hscfg(int argc, char *argv[])
{
	...
	if ((argc == 2) && strstr(argv[1], "-1"))
		strncpy(argv[1], "0xffff", SIZEOF_ARG);
	...
	for (i = 0; i < list->sta_count; i++) {
		...
		printf("\nPower mgmt status: %s\n",
		       (list->info[i].power_mgmt_status == 0) ? "active" :
								"power save");
		printf("Mode: %s\n",
		       (list->info[i].bandmode == BAND_B)   ? "11b," :
		       (list->info[i].bandmode == BAND_G)   ? "11g," :
		       (list->info[i].bandmode == BAND_A)   ? "11a," :
		       (list->info[i].bandmode == BAND_GN)  ? "2.4G_11n," :
		       (list->info[i].bandmode == BAND_AN)  ? "5G_11n," :
		       (list->info[i].bandmode == BAND_GAC) ? "2.4G_11ac," :
...
/**
 *  @brief Show usage information for the cfg_80211d_country_ie command
 *
 *  $return         N/A
 */
static void print_apcmd_cfg_80211d_country_ie_usage(void)
{
	printf("\nUsage: sys_cfg_80211d_country_ie <clear | country Country_code>\n");
	return;
}
/**
...
static int apcmd_cfg_80211d(int argc, char *argv[])
{
	...
	if (argc) {
		...
		if ((output[1][0] != -1) && (output[1][1] > output[1][0])) {
			...
			no_of_sub_band = parse_domain_file(
				country, band, sub_bands, &domain_code, 0);
			if (no_of_sub_band == UAP_FAILURE) {
				printf("Parsing Failed\n");
				return UAP_FAILURE;
			}
	...
	if (cflag) {
		...
		if (domain_code) {
			...
			regDomainInfo.length =
				uap_cpu_to_le16(regDomainInfo.length);
		}
		cmd_buf->domain.length =
			uap_cpu_to_le16(cmd_buf->domain.length);
	}
...
/**
 *  @brief parser for sys_cfg_80211d_country_ie input
 *
 *  @param argc     Number of arguments
 *  @param argv     Pointer to the arguments
 *  @param output   Stores indexes for "country"
 *                  arguments
 *
 *  @return         NA
 *
 */
static void parse_input_80211d_country_ie(int argc, char **argv,
					  int output[1][2])
{
	int i;
	char *keywords[1] = {"country"};

	output[0][0] = -1;
	output[0][1] = -1;

	for (i = 0; i < argc; i++) {
		if (strcmp(argv[i], keywords[0]) == 0)
			output[0][1] = output[0][0] = i;
		output[0][1] += 1;
	}
}
/**
 *  @brief Set/Get 802.11D beacon country IE information
 *
 *  Usage: sys_cfg_80211d_country_ie country_code
 *
 *  @param argc     Number of arguments
 *  @param argv     Pointer to the arguments
 *  @return         UAP_SUCCESS/UAP_FAILURE
 */
static int apcmd_cfg_80211d_country_ie(int argc, char *argv[])
{
	apcmdbuf_cfg_80211d *cmd_buf = NULL;
	ieeetypes_subband_set_t *subband = NULL;
	t_u8 *buf = NULL;
	...
	while ((opt = getopt_long(argc, argv, "+", cmd_options, NULL)) != -1) {
		switch (opt) {
		default:
			print_apcmd_cfg_80211d_country_ie_usage();
			return UAP_SUCCESS;
		}
	}
	argc -= optind;
	...
	if (argc) {
		...
		if (strcmp(argv[0], "country")) {
			printf("ERR: Incorrect input. Country code is needed.");
			print_apcmd_cfg_80211d_country_ie_usage();
			return UAP_FAILURE;
		}
		/** SET */
		parse_input_80211d_country_ie(argc, argv, output);
		...
		if ((output[0][0] != -1) && (output[0][1] > output[0][0])) {
			...
			if ((strlen(argv[output[0][0] + 1]) > 3) ||
			    (strlen(argv[output[0][0] + 1]) == 0)) {
				print_apcmd_cfg_80211d_country_ie_usage();
				return UAP_FAILURE;
			}
			/* Only 2 characters of country code are copied here as
			 * indoor/outdoor conditions are not handled in domain
			 * file */
			strncpy(country, argv[output[0][0] + 1], 2);
			...
			/** Get domain information from the file */
			no_of_sub_band = parse_domain_file(
				country, band, sub_bands, &domain_code, 1);
			if (no_of_sub_band == UAP_FAILURE) {
				printf("Parsing Failed\n");
				return UAP_FAILURE;
			}
	...
	if (argc && !cflag && action != ACTION_CLEAR) {
		printf("ERR: Invalid input\n");
		print_apcmd_cfg_80211d_country_ie_usage();
		return UAP_FAILURE;
	}
	...
	/* Locate headers */
	cmd_buf = (apcmdbuf_cfg_80211d *)buf;
	cmd_len = (argc && action == ACTION_SET) ? cmd_len : /** Set */
						   (sizeof(apcmdbuf_cfg_80211d) -
			    sizeof(domain_param_t)); /** Get */
	...
	cmd_buf->result = 0;
	cmd_buf->seq_num = 0;
	cmd_buf->action = action;
	cmd_buf->action = uap_cpu_to_le16(cmd_buf->action);
	cmd_buf->cmd_code = HostCmd_CMD_802_CUSTOM_BEACON_IE;

	if (cflag) {
		/* third character of country code is copied here as
		 * indoor/outdoor condition is not supported in domain file*/
		strncpy(country, argv[output[0][0] + 1], sizeof(country) - 1);
		for (i = 0; (unsigned int)i < strlen(country); i++) {
			if ((country[i] < 'A') || (country[i] > 'z')) {
				printf("Invalid Country Code\n");
				print_apcmd_cfg_80211d_country_ie_usage();
		...
		if (cmd_buf->result == CMD_SUCCESS) {
			if (argc) {
				if (action == ACTION_CLEAR)
					printf("Custom beacon country IE cleared\n");
				else
					printf("Set executed successfully\n");
			} else {
				j = uap_le16_to_cpu(cmd_buf->domain.length);
			...
			printf("ERR:Command Response incorrect!\n");
			if (argc)
				printf("11d beacon IE info is allowed to set only before bss start.\n");
			ret = UAP_FAILURE;
		}
	} else {
		printf("ERR:Command sending failed!\n");
...
static int update_domain_info(char *country_80211d, int band)
{
	...
	/** Get domain information from the file */
	no_of_sub_band =
		parse_domain_file(country, band, sub_bands, &domain_code, 0);
	if (no_of_sub_band == UAP_FAILURE) {
		printf("Parsing Failed\n");
		ret = UAP_FAILURE;
		goto done;
...
int check_sys_config(t_u8 *buf, t_u16 len)
{
	...
	tlvbuf_wpa3_sae_password *sae_password_tlv = NULL;
	...
	while (tlv_buf_left >= (int)sizeof(tlvbuf_header) &&
		...
		case MRVL_CHANNELCONFIG_TLV_ID:
			...
			if (channel_tlv_band != BAND_A) {
				 ...
				 * channels 5-11 for US and 5-13 for non-US only
				 * for secondary channel below */
...
#define JP_REGION_0xFE 0xFE
		...
		case MRVL_WPA_PASSPHRASE_TLV_ID:
			...
			break;
		case MRVL_WPA3_SAE_PASSWORD_TLV_ID:
			sae_password_tlv =
				(tlvbuf_wpa3_sae_password *)pcurrent_tlv;
			break;
	...
	if (((protocol == PROTOCOL_WPA) || (protocol == PROTOCOL_WPA2) ||
		...
		if (!(passphrase_tlv->length)) {
			printf("ERR:Passphrase must be set for WPA/WPA2/Mixed mode\n");
			return UAP_FAILURE;
		}
	}
	if (protocol == PROTOCOL_WPA3_SAE) {
		if (sae_password_tlv == NULL) {
			printf("ERR: No SAE password found\n");
			return UAP_FAILURE;
		}
		if (!(sae_password_tlv->length)) {
			printf("ERR:Password must be set for WPA3 SAE mode\n");
			return UAP_FAILURE;
		}
	}
...
#define JP_REGION_0xFE 0xFE
	...
	if ((((bss_config->protocol == PROTOCOL_WPA) ||
	    ...
	    ((bss_config->protocol == PROTOCOL_WPA3_SAE) &&
	     !(bss_config->wpa_cfg.sae_password_length))) {
...
{
	printf("\nUsage : uaputl.exe sys_cfg_pmf [MFPC] [MFPR] [BCN_PROT]\n");
...
static int apcmd_sys_cfg_pmf(int argc, char *argv[])
{
	...
	cmd_buf->params.beacon_prot = beacon_prot;
	...
	if (ret == UAP_SUCCESS) {
		if (cmd_buf->result == CMD_SUCCESS) {
			printf("Successfully executed the command\n");
			printf("mfpc: %d, mfpr: %d bcn_prot: %d\n",
			       cmd_buf->params.mfpc, cmd_buf->params.mfpr,
			       cmd_buf->params.beacon_prot);
		} else {
			printf("ERR:Command sending failed!\n");
			free(buf);
			return UAP_FAILURE;
...
int main(int argc, char *argv[])
{
	...
	strncpy(dev_name, DEFAULT_DEV_NAME, sizeof(dev_name));

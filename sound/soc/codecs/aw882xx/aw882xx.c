...
static int aw882xx_i2c_probe(struct i2c_client *i2c)
{
	...
	power_gpio = of_get_named_gpio(i2c->dev.of_node, "power-gpio", 0);
	if (power_gpio < 0) {
		dev_err(&i2c->dev, "no power-gpio provided.");
	} else {
		dev_err(&i2c->dev, "power-gpio provided ok.");
		ret = devm_gpio_request_one(&i2c->dev, power_gpio,
						GPIOF_OUT_INIT_HIGH, "aw882xx_power");
		if (ret) {
			dev_err(&i2c->dev, "power request failed");
			return ret;
		}
	}
